...
parser.add_argument('--filename', type=str, default='center', help='number of neighbors')
...
for shotnum in args.shotnum:
    ...
    for i in range(100):
        ...
        for _ in range(200):
            ...
            if args.dataset in ['ENZYMES', 'PROTEINS']:
                logits = log.forward_homo_graph2(train=1,train_embeds=train_embedding,)
            else:
        ...
        with torch.no_grad():
            if args.dataset in ['ENZYMES', 'PROTEINS']:
                logits = log.forward_homo_graph2(train=0,test_embeds=test_embedding,neighbors=testneighbors,neighbors_2hop=testneighbors_2hop)#train_adj=test_adj)
            else:
